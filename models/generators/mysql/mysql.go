...
func (mysql *MySQL) Generate(_ context.Context, request *module.GeneratorRequest) (*module.GeneratorResponse, error) {
	...
	case LocalDBType:
		resources, err = mysql.GenerateLocalResources(request)
	case CloudDBType:
		...
		case "aws":
			resources, err = mysql.GenerateAWSResources(request)
		case "alicloud":
			resources, err = mysql.GenerateAlicloudResources(request)
		default:
...
func (mysql *MySQL) GenerateDBSecret(request *module.GeneratorRequest, hostAddress, username, password string) (*apiv1.Resource, error) {

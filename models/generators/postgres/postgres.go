...
func (postgres *PostgreSQL) Generate(_ context.Context, request *module.GeneratorRequest) (*module.GeneratorResponse, error) {
	...
	case LocalDBType:
		resources, err = postgres.GenerateLocalResources(request)
	case CloudDBType:
		...
		case "aws":
			resources, err = postgres.GenerateAWSResources(request)
		case "alicloud":
			resources, err = postgres.GenerateAlicloudResources(request)
		default:
...
func (postgres *PostgreSQL) GenerateDBSecret(request *module.GeneratorRequest, hostAddress, username, password string) (*apiv1.Resource, error) {
	...
	resource.Patcher = &apiv1.Patcher{

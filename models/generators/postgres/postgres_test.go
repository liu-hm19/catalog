...
func TestPostgreSQLModule_GenerateDBSecret(t *testing.T) {
	...
	expectedRes, err := module.WrapK8sResourceToKusionResource(resID, sec)
	if err != nil {
		t.Fatalf("failed to wrap secret resource for unit test: %v", err)
	}

	expectedRes.Patcher = &apiv1.Patcher{
	...
	actualRes, err := postgres.GenerateDBSecret(r, hostAddress, username, password)

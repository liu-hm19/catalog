...
func (postgres *PostgreSQL) GenerateAlicloudResources(request *module.GeneratorRequest) ([]apiv1.Resource, error) {
	...
	if region == "" {
		return nil, ErrEmptyAlicloudProviderRegion
	}
	...
	dbSecret, err := postgres.GenerateDBSecret(request, hostAddress, postgres.Username, password)
	if err != nil {
		return nil, err
	}
